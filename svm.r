...
library(kernlab)
tweet_svm_classifier <- ksvm(isfake~., data = training_data, kernel = "vanilladot")
tweet_svm_classifier

tweet_svm_prediction <- predict(tweet_svm_classifier, testing_data)


library(gmodels)
CrossTable(tweet_svm_prediction,testing_data$isfake,prop.chisq = FALSE, prop.t = FALSE, prop.r = FALSE,
           dnn = c('predicted','actual'))
confusion_mat <- table(testing_data$isfake, tweet_svm_prediction)
...
#tweet_svm_results <- table(tweet_svm_prediction, testing_data$isfake)
#tweet_svm_results
#library(caret)
